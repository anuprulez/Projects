...
function generateCalculator(parameter) {
    ...
    function calculator(classNamesArray, operatorsArray, numbersArray, divContainer){
        ...
        var classNamesArray = classNamesArray; // set of class names
        var operatorsArray = operatorsArray;   // set of operators
        var numbersArray = numbersArray;       // set of numbers
        var divContainer = divContainer;       // html div object
        ...
        this.createTableElements = function(){
                ...
                if(numberArrayLength <= 2) {
                    this.createNumberButtons(0, numberArrayLength-1, classNameNumber, tableRow, table);
                }
                // creates operators
                if(operatorArrayLength <= 2) {
                    this.createOperatorButtons(0, operatorArrayLength-1, classNamesArray, operatorsArray, tableRow, table);
                }
    ...
    // handles if 'new' button is clicked
    if(parameter.value === 'new') {
        var newNumbers, newNumberArray, newOperators, newOperatorsArray;
        // gets the sets of numbers and operators form the textbox
        newNumbers = document.getElementById('txtNumber').value;
        newOperators = document.getElementById('txtOperator').value;
        // converts comma separated inputs in textboxes into an array format
        if(newNumbers !== "") {
            newNumberArray = newNumbers.split(',');
        }
        if(newOperators !== "") {
            newOperatorsArray = newOperators.split(',');
        }

        // definition of child class
        function NewCalculator(message) {
            var message = message;
            // clears the div
        ...
        // inheriting from calculator base class
        NewCalculator.prototype = new calculator(classArray, newOperatorsArray, newNumberArray, divContainer);
        // changing the prototype constructor of the child class of child class type
        NewCalculator.prototype.constructor = NewCalculator;
        // setting the calculator as baseClass
        NewCalculator.prototype.baseClass = calculator.prototype.constructor;

        // instantiating the NewCalulator child class
        var calc = new NewCalculator('My customized calculator');
        // clears the div
        calc.clearDiv();
        // creates calculator based on the input give
...
function getButtonClickedValue(btnClickValue) {
    ...
    function getValue(txtBoxObj, valueButton, buttonClassName) {
          ...
          this.verifyExpression = function(operator) {
                ...
                var resultLength =  this.getTextboxLength();
          ...
          this.distinguishButtonClick = function(btnClass) {
                switch (btnClass) {
                  ...
                  case "clsButton clsBack":
                      ...
                      if(this.getTextboxLength() > 0) {
                          textBoxObject.value = textBoxObject.value.substring(0, this.getTextboxLength() - 1);
                      }
                  break;
